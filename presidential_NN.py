...
clf_model = Sequential([
    Dense(16, activation='relu', input_shape=(X_train_clf.shape[1],)),
    Dense(8, activation='relu'),
    Dense(2, activation='softmax')  # 2 clases: Democrat y Republican
])

clf_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

# Entrenar el modelo
clf_model.fit(X_scaled, df['winning_party_encoded'], epochs=50, batch_size=8, verbose=0)
...
reg_model = Sequential([
    Dense(16, activation='relu', input_shape=(X_train_reg.shape[1],)),
    Dense(8, activation='relu'),
...
# Entrenar el modelo
reg_model.fit(X_scaled, df['democrat_percentage'], epochs=50, batch_size=8, verbose=0)
